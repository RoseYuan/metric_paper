...
```{r}
st <- 0.5
p4 <- dfl %>% filter(metric == "SpatialSPA_neg") %>% filter(k %in% c("k=6","k=18")) %>%
  ggplot(aes(x = col, y = -row, color = value)) +
  geom_point(size=s, stroke = st) + 
  labs(x = "", y = "", color="") +
  theme_minimal() +
...
```{r}
st <- 0.5
p8 <- dfl %>% filter(metric == "SpatialSPA") %>% filter(k %in% c("k=6","k=18")) %>%
  ggplot(aes(x = col, y = -row, color = value)) +
  geom_point(size=s, stroke = st) + 
  labs(x = "", y = "", color="") +
  theme_minimal() +
...
p7 <- cowplot::plot_grid(p6, p4, 
                         rel_widths=c(1,3.2), ncol=2, 
                         labels=c("a","b"), scale=c(1, 1))

p10 <- cowplot::plot_grid(p9, p8, 
                         rel_widths=c(1,3.2), ncol=2, 
                         labels=c("c","d"), scale=c(1, 1))

pdf("spatial_metrics_real_data_spa.pdf", width=9.5, height=5)
cowplot::plot_grid(p7, p10,
                   rel_widths=c(1,3.2), nrow=2, 
                   labels=c("",""))
dev.off()
```
