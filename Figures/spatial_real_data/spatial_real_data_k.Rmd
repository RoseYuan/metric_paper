...
for (k in c(6, 18, 30, 42)) {
  ...
  agg <- getSpatialExternalMetrics(true, clusterlabels, location, k=k, level="element", useNegatives = FALSE, metrics=c("SpatialSPA"), fuzzy_true = TRUE, fuzzy_pred = FALSE)
  tmp$"SpatialSPA" <- agg$SpatialSPA
  agg <- getPartitionMetrics(true, clusterlabels, level="element", useNegatives = FALSE, metrics=c("SPA"))
  tmp$"SPA" <- agg$SPA
  agg <- getSpatialExternalMetrics(true, clusterlabels, location, k=k, level="element", useNegatives = TRUE, metrics=c("SpatialSPA"), fuzzy_true = TRUE, fuzzy_pred = FALSE)
  tmp$"SpatialSPA_neg" <- agg$SpatialSPA
  agg <- getPartitionMetrics(true, clusterlabels, level="element", useNegatives = TRUE, metrics=c("SPA"))
  tmp$"SPA_neg" <- agg$SPA
  tmp$k <- paste0("k=",k)
  tmp$row <- df$row
  tmp$col <- df$col
...
```{r}
dfl <- as.data.frame(res %>% pivot_longer(cols = c("PAS", "ELSA", "ELSA.Ea", "ELSA.Ec", "SpatialSPA", "SPA", "SpatialSPA_neg","SPA_neg"), names_to="metric", values_to = "value"))
...
```{r}
st <- 0.2
p2 <- dfl %>% filter(metric == "PAS") %>% filter(k %in% c("k=6","k=18","k=42")) %>%
...
```{r}
st <- 0.5
p4 <- dfl %>% filter(metric == "SpatialSPA_neg") %>% filter(k != "k=30") %>%
  ggplot(aes(x = col, y = -row, color = value)) +
  geom_point(size=s, stroke = st) + 
  labs(x = "", y = "", color="") +
  theme_minimal() +
  ...
  # guides(color = guide_legend(keywidth = 1, keyheight = 0.8, override.aes = list(size = 3)),
  #        shape = guide_legend(keywidth = 1, keyheight = 0.8, override.aes = list(size = 3))) +
  labs(color="SPA\n(use\nnegatives)", title="Spatial SPA (use negatives)")

p4
```

```{r}
p6 <- dfl %>% filter(metric == "SPA_neg") %>% filter(k == "k=6") %>%
  ggplot() +
  ...
  labs(color="SPA",title="NonSpatial SPA\n(use negatives)")
p6
```

```{r}
max(dfl %>% filter(metric == "SpatialSPA") %>% filter(k != "k=30") %>% select(value))
```

```{r}
st <- 0.5
p8 <- dfl %>% filter(metric == "SpatialSPA") %>% filter(k != "k=30") %>%
  ggplot(aes(x = col, y = -row, color = value)) +
  geom_point(size=s, stroke = st) + 
  labs(x = "", y = "", color="") +
  theme_minimal() +
  ...
  labs(color="SPA", title="Spatial SPA")

p8
```

```{r}
p9 <- dfl %>% filter(metric == "SPA") %>% filter(k == "k=6") %>%
  ggplot() +
  geom_point(aes(x = col, y = -row, color = value), size = s) +
  labs(x = "", y = "", color="") +
  theme_minimal() +
...
p5 <- cowplot::plot_grid(p1, p2, rel_widths=c(1,3.2), ncol=2, labels=letters[1:2], scale=c(1, 1))
p7 <- cowplot::plot_grid(p6, 
                         p4, 
                         rel_widths=c(1,3.2), ncol=2, labels=c("e",""), scale=c(1, 1))
p10 <- cowplot::plot_grid(p9, 
                         p8, 
                         rel_widths=c(1,3.2), ncol=2, labels=c("",""), scale=c(1, 1))
p33 <- cowplot::plot_grid(p0, 
                         p3, 
                         rel_widths=c(1,3.2), ncol=2, labels=c("c","d"), scale=c(1, 1))
cowplot::plot_grid(p5, p33, p7, rel_heights=c(1,1,1), ncol=1, labels=c("","",""), scale=c(1, 1))
dev.off()
```
```{r}
pdf("spatial_metrics_real_data_spa_withoutN.pdf", width=9.5, height=2.5)
p10
dev.off()
```
