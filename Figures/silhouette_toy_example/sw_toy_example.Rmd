...
gm <- getGlobal(da)
gm <- data.frame(scenario=rep(as.character(1:2), each=5),
                 metric=rep(names(gm),2),
                 value=c(gm, getGlobal(d3)))
gmp <- ggplot(gm, aes(x=scenario, y=value, fill=scenario)) +
  geom_col(position = "dodge") + facet_wrap(~metric, scales="free_y", ncol=2) + 
  theme_classic() + theme(legend.position=c(1, 0), axis.title.x=element_blank(),
